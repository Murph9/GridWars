...
public class MyTexture {
	...
	public MyTexture(GL2 gl, String fileName, boolean mipmaps) {
        ...
        gl.glTexImage2D(GL.GL_TEXTURE_2D, 0, data.getInternalFormat(), data.getWidth(),
        				data.getHeight(), 0, data.getPixelFormat(), data.getPixelType(),
        				data.getBuffer());
	...
	public MyTexture(GL2 gl, ByteBuffer buffer, int size) {
		gl.glGenTextures(1, textureID, 0);
		gl.glBindTexture(GL.GL_TEXTURE_2D, textureID[0]);
